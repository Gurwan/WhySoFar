...
if(url.includes("boulanger.com")){
    ...
    li.forEach(function(el){
        if(el.getElementsByClassName("is--bold")[0] != undefined && el.getElementsByClassName("is--bold")[0].innerHTML.toLowerCase().includes("poids")){
            poids = el.outerText.split(":")[1];
        }

        if(el.getElementsByClassName("is--bold")[0] != undefined && el.getElementsByClassName("is--bold")[0].innerHTML.toLowerCase().includes("fabriqué en")){
            origineCountry = el.outerText.split(":")[1];
        }
    })

    if(origineCountry == 'inconnu'){
        var bl = document.querySelectorAll("bl-viewer");
        bl.forEach(function(el){
            if(el.className == "viewer"){
                ean = el.getAttribute('data-ean');
            }
        });
    }

    var priceDiv = document.getElementsByClassName("product__price-wrapper")[0];
    if(priceDiv == null){
        priceDiv = document.getElementsByClassName("price")[0]
    }
    ...
    divInd.style.position = "relative";
